...
public class MemoriesConditionInteraction extends Interaction {
   ...
   public static MemoriesConditionInteraction deserialize(@Nonnull ByteBuf buf, int offset) {
      if (buf.readableBytes() - offset < 39) {
         ...
         if ((nullBits & 32) != 0) {
            ...
            int varIntLenx = VarInt.size(memoriesNextCount);
            ...
            for (int ix = 0; ix < memoriesNextCount; ix++) {
   ...
   public static MemoriesConditionInteraction toObject(MemorySegment mem, int offset) {
      if (offset + 39 > mem.byteSize()) {
         ...
         int[] tags = null;
         if (hasTags(mem, offset)) {
            int offx = offset + getValidatedOffset(mem, offset, 27, 39, "Tags");
         ...
         if (hasMemoriesNext(mem, offset)) {
            int offxx = offset + getValidatedOffset(mem, offset, 35, 39, "MemoriesNext");
            long packedxx = VarInt.getWithLength(mem, offxx);
            int lenxx = (int)packedxx;
            if (lenxx < 0) {
               throw ProtocolException.negativeLength("MemoriesNext", lenxx);
            }

            if (lenxx > 4096000) {
               throw ProtocolException.dictionaryTooLarge("MemoriesNext", lenxx, 4096000);
            }

            memoriesNext = new HashMap<>(lenxx);
      ...
      if (buffer.readableBytes() - offset < 39) {
         ...
         if (v >= 3) {
            ...
            if ((nullBits & 32) != 0) {
               ...
               int posxx = offset + 39 + v;
               int memoriesNextCount = VarInt.peek(buffer, posxx);
               if (memoriesNextCount < 0) {
               ...
               posxx += VarInt.size(memoriesNextCount);
