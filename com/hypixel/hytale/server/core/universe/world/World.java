...
import com.hypixel.hytale.metrics.ExecutorMetricsRegistry.ExecutorMetric;
...
import com.hypixel.hytale.server.core.prefab.selection.buffer.impl.IPrefabBuffer.RawBlockConsumer;
import com.hypixel.hytale.server.core.receiver.IMessageReceiver;
import com.hypixel.hytale.server.core.universe.PlayerRef;
import com.hypixel.hytale.server.core.universe.Universe;
import com.hypixel.hytale.server.core.universe.world.World.1;
import com.hypixel.hytale.server.core.universe.world.World.2;
import com.hypixel.hytale.server.core.universe.world.World.GenericBlockBulkUpdater;
...
import com.hypixel.hytale.server.core.util.FillerBlockUtil.ValidationResult;
...
public class World extends TickingThread implements Executor, ExecutorMetric, ChunkAccessor<WorldChunk>, IWorldChunks, IMessageReceiver {
    public static final float SAVE_INTERVAL = 10.0F;
    public static final String DEFAULT = "default";
    @Nonnull
    public static final ExecutorMetricsRegistry<World> METRICS_REGISTRY = new ExecutorMetricsRegistry()
    ...
    public World(@Nonnull String name, @Nonnull Path savePath, @Nonnull WorldConfig worldConfig) throws IOException {
        ...
        ArmorVisibilityOption armorVisibilityOption = gameplayConfig.getPlayerConfig().getArmorVisibilityOption();
    ...
    public void stopIndividualWorld(Map<UUID, PlayerRef> players) {
        ...
        if (this.alive.getAndSet(false)) {
            try {
                ...
                ((Api)this.logger.at(Level.SEVERE).withCause(var6)).log("Exception while shutting down world:");
            }
        }
    }
    ...
    public void validateDeleteOnRemove() {
        if (this.worldConfig.isDeleteOnRemove()) {
            try {
                ...
                ((Api)this.logger.at(Level.SEVERE).withCause(var2)).log("Exception while deleting world on remove:");
            }
        }
    }
    ...
    public void setTps(int tps) {
        ...
        this.entityStore
            ...
            .forEachEntityParallel(
                ...
                (index, archetypeChunk, commandBuffer) -> ((PlayerRef)archetypeChunk.getComponent(index, PlayerRef.getComponentType()))
                        .getPacketHandler()
    ...
    public <T> void getBlockBulkRelative(
        ...
        @Nonnull GenericBlockBulkUpdater<T> consumer
    ) {
    ...
    public WorldChunk getChunkIfInMemory(long index) {
        ...
        if (reference == null) {
            ...
            return !this.isInThread()
                ? (WorldChunk)CompletableFuture.supplyAsync(() -> this.getChunkIfInMemory(index), this).join()
                : (WorldChunk)this.chunkStore.getStore().getComponent(reference, WorldChunk.getComponentType());
        }
    }
    ...
    public CompletableFuture<WorldChunk> getChunkAsync(long index) {
        return this.chunkStore
            ...
            .thenApplyAsync(
                reference -> reference == null ? null : (WorldChunk)this.chunkStore.getStore().getComponent(reference, WorldChunk.getComponentType()), this
            );
    }
    ...
    public <T extends Entity> T addEntity(T entity, @Nonnull Vector3d position, @Nullable Vector3f rotation, @Nonnull AddReason reason) {
        if (!EntityModule.get().isKnown(entity)) {
            ...
            ((Api)this.logger.at(Level.WARNING).withCause(new SkipSentryException())).log("Warning addEntity was called off thread!");
    ...
    public void consumeTaskQueue() {
        ...
        while ((runnable = (Runnable)this.taskQueue.poll()) != null) {
            try {
                ...
                ((Api)this.logger.at(Level.SEVERE).withCause(var9)).log("Failed to run task!");
            }
        }
    }
    ...
    public CompletableFuture<PlayerRef> addPlayer(
        ...
        if (!this.alive.get()) {
            ...
            if (!packetHandler.stillActive()) {
                ...
                AddPlayerToWorldEvent event = (AddPlayerToWorldEvent)HytaleServer.get()
                    .getEventBus()
                ...
                return CompletableFuture.allOf(setupPlayerFuture, playerReadyFuture, loadTargetChunkFuture)
                    ...
                    .exceptionally(throwable -> {
                        ((Api)this.logger.at(Level.WARNING).withCause(throwable)).log("Exception when adding player to world!");
    ...
    public CompletableFuture<Void> drainPlayersTo(@Nonnull World fallbackTargetWorld, Collection<PlayerRef> players) {
        ...
        for (PlayerRef playerRef : players) {
            ...
            DrainPlayerFromWorldEvent event = (DrainPlayerFromWorldEvent)HytaleServer.get()
                .getEventBus()
    ...
    public void validate(@Nonnull StringBuilder errors, @Nonnull RawBlockConsumer<Void> blockValidator, @Nonnull EnumSet<ValidationOption> options) throws IOException {
        ...
        while (var6.hasNext()) {
            ...
            try {
                ...
                Ref<ChunkStore> reference = (Ref<ChunkStore>)future.join();
                if (reference != null && reference.isValid()) {
                    ...
                    if (chunkColumn != null) {
                        for (Ref<ChunkStore> section : chunkColumn.getSections()) {
                            ChunkSection sectionInfo = (ChunkSection)store.getComponent(section, ChunkSection.getComponentType());
                                    ...
                                    for (int z = 0; z < 32; z++) {
                                        for (int x = 0; x < 32; x++) {
                                            ...
                                            if (options.contains(ValidationOption.BLOCK_FILLER)) {
                                                1 fetcher = new 1(this, sectionInfo);
                                                ValidationResult fillerResult = FillerBlockUtil.validateBlock(
                                                    x, y, z, blockId, rotation, filler, blockSection, this.chunkStore, fetcher
                                                );
                                                switch (2.$SwitchMap$com$hypixel$hytale$server$core$util$FillerBlockUtil$ValidationResult[fillerResult.ordinal()]) {
                                                    case 1:
                                                    default:
                                                        break;
                                                    case 2: {
                        ...
                        if (options.contains(ValidationOption.ENTITIES)) {
                            ...
                            for (Holder<EntityStore> entityHolder : chunk.getEntityChunk().getEntityHolders()) {
                                UnknownComponents<EntityStore> unknownComponents = (UnknownComponents<EntityStore>)entityHolder.getComponent(unknownComponentType);
                ...
                ((Api)this.getLogger().at(Level.SEVERE).withCause(var35)).log("Failed to validate chunk: %d, %d", chunkX, chunkZ);
                errors.append('\t')
