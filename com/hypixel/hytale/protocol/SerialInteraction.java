...
public class SerialInteraction extends Interaction {
   ...
   public static SerialInteraction deserialize(@Nonnull ByteBuf buf, int offset) {
      if (buf.readableBytes() - offset < 35) {
         ...
         if ((nullBits & 32) != 0) {
            ...
            if (varPos5 + varIntLenx + serialInteractionsCount * 4L > buf.readableBytes()) {
               throw ProtocolException.bufferTooSmall("SerialInteractions", varPos5 + varIntLenx + serialInteractionsCount * 4, buf.readableBytes());
            }

            obj.serialInteractions = new int[serialInteractionsCount];

            for (int ix = 0; ix < serialInteractionsCount; ix++) {
               obj.serialInteractions[ix] = buf.getIntLE(varPos5 + varIntLenx + ix * 4);
            }
         }
   ...
   public static SerialInteraction toObject(MemorySegment mem, int offset) {
      if (offset + 35 > mem.byteSize()) {
         ...
         int[] tags = null;
         if (hasTags(mem, offset)) {
            int offx = offset + getValidatedOffset(mem, offset, 23, 35, "Tags");
         ...
         if (hasSerialInteractions(mem, offset)) {
            int offxx = offset + getValidatedOffset(mem, offset, 31, 35, "SerialInteractions");
            long packedxx = VarInt.getWithLength(mem, offxx);
            int lenxx = (int)packedxx;
            if (lenxx < 0) {
               throw ProtocolException.negativeLength("SerialInteractions", lenxx);
            }

            if (lenxx > 4096000) {
               throw ProtocolException.arrayTooLong("SerialInteractions", lenxx, 4096000);
            }

            int lenOffset = (int)(packedxx >>> 32);
            if (offxx + lenOffset + lenxx * 4L > mem.byteSize()) {
               throw ProtocolException.bufferTooSmall("SerialInteractions", offxx + lenOffset + lenxx * 4, (int)mem.byteSize());
            }

            offxx += lenOffset;
            serialInteractions = new int[lenxx];
            MemorySegment.copy(mem, PacketIO.PROTO_INT, offxx, serialInteractions, 0, lenxx);
         }
   ...
   public int serialize(@Nonnull MemorySegment mem, int offset) {
      ...
      if (this.effects != null) {
         mem.set(PacketIO.PROTO_INT, (long)(offset + 11), varOffset - offset - 35);
      ...
      if (this.settings != null) {
         mem.set(PacketIO.PROTO_INT, (long)(offset + 15), varOffset - offset - 35);
         if (this.settings.size() > 4096000) {
            throw ProtocolException.dictionaryTooLarge("Settings", this.settings.size(), 4096000);
         }
      ...
      if (this.rules != null) {
         mem.set(PacketIO.PROTO_INT, (long)(offset + 19), varOffset - offset - 35);
      ...
      if (buffer.readableBytes() - offset < 35) {
         ...
         if (v >= 3) {
            ...
            if ((nullBits & 32) != 0) {
               ...
               int posxx = offset + 35 + v;
               int serialInteractionsCount = VarInt.peek(buffer, posxx);
               if (serialInteractionsCount < 0) {
               ...
               posxx += VarInt.size(serialInteractionsCount);
               posxx += serialInteractionsCount * 4;
