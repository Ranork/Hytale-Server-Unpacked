...
public class BlockConditionInteraction extends SimpleBlockInteraction {
   ...
   public static BlockConditionInteraction deserialize(@Nonnull ByteBuf buf, int offset) {
      if (buf.readableBytes() - offset < 44) {
         ...
         if ((nullBits & 32) != 0) {
            ...
            int varIntLenx = VarInt.size(matchersCount);
            if (matchersCount > 4096000) {
               throw ProtocolException.arrayTooLong("Matchers", matchersCount, 4096000);
            }

            if (varPos5 + varIntLenx + matchersCount * 3L > buf.readableBytes()) {
               throw ProtocolException.bufferTooSmall("Matchers", varPos5 + varIntLenx + matchersCount * 3, buf.readableBytes());
            }
   ...
   public static BlockConditionInteraction toObject(MemorySegment mem, int offset) {
      if (offset + 44 > mem.byteSize()) {
         ...
         int[] tags = null;
         if (hasTags(mem, offset)) {
            int offx = offset + getValidatedOffset(mem, offset, 32, 44, "Tags");
         ...
         if (hasMatchers(mem, offset)) {
            int offxx = offset + getValidatedOffset(mem, offset, 40, 44, "Matchers");
            long packedxx = VarInt.getWithLength(mem, offxx);
            int lenxx = (int)packedxx;
            if (lenxx < 0) {
               throw ProtocolException.negativeLength("Matchers", lenxx);
            }

            if (lenxx > 4096000) {
               throw ProtocolException.arrayTooLong("Matchers", lenxx, 4096000);
            }

            int lenOffset = (int)(packedxx >>> 32);
            if (offxx + lenOffset + lenxx > mem.byteSize()) {
               throw ProtocolException.bufferTooSmall("Matchers", offxx + lenOffset + lenxx, (int)mem.byteSize());
            }

            offxx += lenOffset;
            matchers = new BlockMatcher[lenxx];

            for (int ix = 0; ix < lenxx; ix++) {
               matchers[ix] = BlockMatcher.toObject(mem, offxx);
               offxx += matchers[ix].computeSize();
            }
         }
   ...
   public int serialize(@Nonnull MemorySegment mem, int offset) {
      ...
      if (this.effects != null) {
         mem.set(PacketIO.PROTO_INT, (long)(offset + 20), varOffset - offset - 44);
      ...
      if (this.settings != null) {
         mem.set(PacketIO.PROTO_INT, (long)(offset + 24), varOffset - offset - 44);
         if (this.settings.size() > 4096000) {
            throw ProtocolException.dictionaryTooLarge("Settings", this.settings.size(), 4096000);
         }
      ...
      if (this.rules != null) {
         mem.set(PacketIO.PROTO_INT, (long)(offset + 28), varOffset - offset - 44);
      ...
      if (this.tags != null) {
         mem.set(PacketIO.PROTO_INT, (long)(offset + 32), varOffset - offset - 44);
         if (this.tags.length > 4096000) {
            throw ProtocolException.arrayTooLong("Tags", this.tags.length, 4096000);
         }
      ...
      if (this.camera != null) {
         mem.set(PacketIO.PROTO_INT, (long)(offset + 36), varOffset - offset - 44);
      ...
      if (this.matchers != null) {
         mem.set(PacketIO.PROTO_INT, (long)(offset + 40), varOffset - offset - 44);
         if (this.matchers.length > 4096000) {
            throw ProtocolException.arrayTooLong("Matchers", this.matchers.length, 4096000);
         }
      ...
      if (buffer.readableBytes() - offset < 44) {
         ...
         if (v >= 3) {
            ...
            if ((nullBits & 32) != 0) {
               ...
               int posxx = offset + 44 + v;
               int matchersCount = VarInt.peek(buffer, posxx);
               if (matchersCount < 0) {
               ...
               posxx += VarInt.size(matchersCount);

               for (int i = 0; i < matchersCount; i++) {
                  ValidationResult structResult = BlockMatcher.validateStructure(buffer, posxx);
                  if (!structResult.isValid()) {
                     return ValidationResult.error("Invalid BlockMatcher in Matchers[" + i + "]: " + structResult.error());
                  }

                  posxx += BlockMatcher.computeBytesConsumed(buffer, posxx);
               }
            }
