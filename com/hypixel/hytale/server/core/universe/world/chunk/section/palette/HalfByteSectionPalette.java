...
public final class HalfByteSectionPalette extends AbstractSectionPalette {
   ...
   public int deserialize(AbstractSectionPalette.KeyMemoryDeserializer deserializer, MemorySegment memorySegment, int baseOffset) {
      ...
      if (internalIdRemapping != null) {
         for (int ix = 0; ix < 32768; ix++) {
            byte old = BitUtil.getNibble(this.blocks, ix);
            if (internalIdRemapping.containsKey(old)) {
               BitUtil.setNibble(this.blocks, ix, internalIdRemapping.get(old));
            }
         }
      }
   ...
   public static HalfByteSectionPalette fromBytePalette(@Nonnull ByteSectionPalette section) {
      if (section.count() > 16) {
         ...
         newBlocks = section.internalIdCount.byte2ShortEntrySet().iterator();

         while (newBlocks.hasNext()) {
            it.unimi.dsi.fastutil.bytes.Byte2ShortMap.Entry entry = (it.unimi.dsi.fastutil.bytes.Byte2ShortMap.Entry)newBlocks.next();
            internalIdCount.put(internalIdRemapping.get(entry.getByteKey()), entry.getShortValue());
         }

         byte[] newBlocksx = new byte[16384];
         ...
         for (int i = 0; i < section.blocks.length; i++) {
            ...
            BitUtil.setNibble(newBlocksx, i, byteInternalId);
         }

         return new HalfByteSectionPalette(externalToInternal, internalToExternal, internalIdSet, internalIdCount, newBlocksx);
      }
   }
}
