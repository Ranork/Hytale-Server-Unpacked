...
public class TriggerVolumeDisableCommand extends AbstractWorldCommand {
   private final RequiredArg<String> nameArg = this.withRequiredArg(
      "name", "server.commands.triggervolume.disable.name.desc", TriggerVolumeArgTypes.VOLUME_NAME
   );

   public TriggerVolumeDisableCommand() {
      super("disable", "server.commands.triggervolume.disable.desc");
   }
   ...
   protected void execute(@Nonnull CommandContext context, @Nonnull World world, @Nonnull Store<EntityStore> store) {
      ...
      if (manager != null) {
         ...
         if (entry == null) {
            ...
            context.sendMessage(Message.translation("server.commands.triggervolume.disable.success").param("name", name));
         }
      }
   }
}
